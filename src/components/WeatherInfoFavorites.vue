...
const current = WeatherService.current(props.query);
...
const date = computed(() =>
  current.data.value?.dt
    ? new Date((current.data.value.dt || 0) * 1000).toLocaleDateString('en-EN', {
        weekday: 'short',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
...
<template>
  <div :class="[$style.main, { [$style.selected]: selected }]" @click="emit('select', name)">
